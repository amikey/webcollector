 ...
 */
@Component
public class HXShenGouGongGaoPageCrawler extends AbstractBaseCrawler {

    private static Logger logger = LoggerFactory.getLogger(HXShenGouGongGaoPageCrawler.class);
    ...
    static {
        startUrls.add("http://www.huaxiacae.com/notice/sggg/");
    }
    ...
    public void process(Page page) {//http://www.nfqbyp.com/infomation.html?pageIndex=2&newsTypeID=16793&newsType=%E4%B8%AD%E5%BF%83%E9%80%9A%E5%91%8A
        ...
        if(html.indexOf("JumpSelf")>0 && html.indexOf("self.location=\"")>0 && html.length()<500){
            html = html.split("self.location=\"/notice/sggg/")[1].split("\"")[0];
            page.addTargetRequest("http://www.huaxiacae.com/notice/sggg/" + html);
            return;
        }
